        ...
        <div>
            ...
            <div class="row dropdown my-2">
                <button class="btn btn-success dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <span id="dropdown-name">Filtrar</span>
                </button>
                <ul class="dropdown-menu">
                    <form action="{{ route('word.index') }}" method="get">
                        @csrf
                        <li class="dropdown-item" data-category-id="0" style="color: gray"><button class="btn">Todas las
                                categorías</button></li>
                    </form>
                        ...
                        @if ($category->language_id === session('selected_language_id'))
                            <form action="{{ route('word.filter', ['category' => $category->id]) }}" method="post">
                                ...
                                <li class="dropdown-item" data-category-id="{{ $category->id }}"
                                    style="color:{{ $category->color }}"><button
                                        class="btn">{{ $category->category }}</button></li>
                            </form>
                        @endif
                    @endforeach
                </ul>
